...
- [VirtualBox + Ubuntu](#virtualbox--ubuntu)
    - [VirtualBoxの設定とゲストOS（Ubuntu）のインストール](#virtualboxの設定とゲストosubuntuのインストール)
        - [ファイルのダウンロード](#ファイルのダウンロード)
            - [VirtualBox](#virtualbox)
            - [UbunutuのISOイメージ](#ubunutuのisoイメージ)
            - [CentOSのISOイメージ](#centosのisoイメージ)
        - [VirtualBoxのインストール](#virtualboxのインストール)
            - [<a name="extpack">拡張パック</a>のインストール](#a-nameextpack拡張パックaのインストール)
        - [仮想マシンの作成（Ubuntu/CentOS共通）](#仮想マシンの作成ubuntucentos共通)
            - [名前とオペレーションシステムを入力して`次へ(N)`をクリック](#名前とオペレーションシステムを入力して次へnをクリック)
            - [メモリーサイズを入力して`次へ(N)`をクリック](#メモリーサイズを入力して次へnをクリック)
            - [ハードディスクの作成](#ハードディスクの作成)
            - [<a name="portforwarding">ホストOSから接続するための設定</a>](#a-nameportforwardingホストosから接続するための設定a)
            - [USBの設定](#usbの設定)
        - [<a name="ubuntuinstall">ゲストOS（Ubuntu）のインストール</a>](#a-nameubuntuinstallゲストosubuntuのインストールa)
            - [言語の選択](#言語の選択)
            - [インストール用の設定](#インストール用の設定)
            - [再起動](#再起動)
            - [再起動後の設定](#再起動後の設定)
            - [追加のインストールと更新](#追加のインストールと更新)
            - [更新後の再起動](#更新後の再起動)
            - [Guest Additionsのインストール](#guest-additionsのインストール)
        - [Ubuntuの設定](#ubuntuの設定)
            - [画面の解像度変更](#画面の解像度変更)
            - [クリップボードの共有](#クリップボードの共有)
        - [スナップショットの活用](#スナップショットの活用)
            - [スナップショットの作成](#スナップショットの作成)
            - [スナップショットの復元](#スナップショットの復元)
